...
for current_file in all_files: 
	if (current_file.endswith(".tgz")) or (current_file.endswith("tar.gz")):
		...
		if os.path.exists(file_name):
			shutil.rmtree(file_name) #remove if already exists
				...
				with open(my_file1, errors='ignore') as my_file2:
					for line_no, line in enumerate(my_file2):
						if string_to_search in line:
							print("String is found in " + my_file1 + "; Line Number = " + str(line_no))
		if os.path.exists(file_name):
			shutil.rmtree(file_name) #remove after extraction and search done